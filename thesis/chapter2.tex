...
A blockchain is a specific implementation of DLT wherein data is structured into sequential batches known as "blocks." As defined in the Bitcoin whitepaper \cite{nakamoto2008bitcoin}, this structure utilizes cryptographic chaining to ensure the tamper-proofing of historical data, ordering, and immutability.
...
The fundamental challenge of digital currency is that digital data is easily reproducible. Unlike physical cash, where handing over a bill ends possession, a digital token is information that could theoretically be sent to multiple recipients simultaneously. This is known as the \textbf{Double-Spending Problem}. 
...
In peer-to-peer (P2P) networks, a malicious party may attempt to gain disproportionate control by creating and operating a large number of fake or pseudonymous identities. This vector of attacks is known as a \emph{Sybil attack} \cite{douceur2002sybil}. By posing as multiple independent participants, a single attacker can divert the network mechanisms that assume a one-to-one correspondence between network identities and real-world entities.
...
Blockchain systems mitigate the Sybil attack vector by explicitly binding participation and decision-making power to scarce and verifiable resources. In Proof-of-Work (PoW) systems, influence is proportional to computational power, making large-scale identity forgery economically costly. Similarly, Proof-of-Stake (PoS) systems tie influence to the financial capital locked within the protocol. By replacing identity-based trust systems with resource-based trust systems, blockchain protocols significantly increase the cost of Sybil attacks and render them economically unfeasible under rational adversarial behavior.
...
As a result, once a transaction has been successfully processed and its nonce consumed, any subsequent replay of the same transaction is rejected by honest nodes as invalid. This mechanism guarantees transaction freshness and preserves the integrity of the global state despite any message replay attempts \cite{zhou2025blockchainsecuritybasedcryptography}.
...
Because hash functions are designed for high sensitivity (Avalanche Effect), the only strategy to find a valid $n$ is via brute-force search. This process requires significant CPU cycles to compute. Since calculating the hash of input data is computationally easy and efficient, deriving the original input data from a specific hash value is computationally infeasible. Consequently, the resulting “proof” is trivial for other nodes to verify with a single hash operation.
...
\textbf{Analysis:} While PoW provides the highest level of security and decentralization, its primary drawbacks are its \textbf{environmental impact} and \textbf{low throughput}, as the block interval and size are strictly limited to allow for global synchronization \cite{croman2016scaling}.
...
Delegated Proof of Stake (DPoS) \cite{larimer2014dpos, Consensus2023DPoS}, represents a shift from purely algorithmic consensus to a \textbf{representative democratic model}. It is designed specifically to overcome the scalability bottlenecks inherent in traditional BFT and PoW systems by concentrating block production among a small set of elected nodes.
...
The primary technical advantage of DPoS lies in its reduction of network overhead. The communication complexity required for nodes to reach a consensus on a single block is $O(n^2)$, where $n$ is the total number of participants. This quadratic growth occurs because every node must broadcast its "prepare" and "commit" messages to every other node in the network. As $n$ grows into the thousands, the time required for message propagation and processing leads to significant latency \cite{CastroLiskov1999}. 
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|X|}
        ...
        \textbf{Throughput} 
        & Approx. 7 TPS \cite{croman2016scaling}
        & Approx. 15--30 TPS \cite{ethereum_pos}
        & $>$4,000 TPS \cite{springer2025_dpos_scalability}
        & $>$50,000 TPS \cite{yakovenko2018poh} \\ \hline
...
\section{Decentralized Data Storage} \label{ch2_storage}

While blockchains excel at maintaining an immutable order of small state data, they are inefficient for storing large datasets. This limitation requires the integration of off-chain decentralized storage solutions in order to build scalable applications.

\subsection{The Data Availability Problem} \label{ch2_data_availability}
In a blockchain system, every full node must download and store the entire history of the blockchain to validate new blocks. This creates a problem where the cost of storage is multiplied by the number of nodes in the network. This makes storing large files directly on the blockchain expensive, leading to a significant increase in blockchain size. Consequently, this increases the synchronization time for new nodes and centralizes the network by raising the hardware barrier to entry \cite{benet2014ipfs}.

\subsection{InterPlanetary File System (IPFS)}
The InterPlanetary File System (IPFS) is a peer-to-peer hypermedia protocol designed to address these storage limitations by creating a content-addressable distributed storage network \cite{benet2014ipfs}.

\subsubsection{Content Addressing vs. Location Addressing}
Traditional web protocols (HTTP) utilize location addressing, where data is identified by \textit{where} it is hosted. If the server goes offline or the file is moved, the link breaks.

In contrast, IPFS utilizes content addressing, where data is identified by \textit{what} it is, specifically by its cryptographic hash (CID). When a user requests a file with hash $H$, the network locates the nodes that possess the data matching $H$, which ensures:
\begin{itemize}
    \item \textbf{Immutability:} If the content of the file changes, its hash changes, creating a new address.
    \item \textbf{Deduplication:} Identical files produce the exact same hash and are stored only once on the network.
\end{itemize}

\subsubsection{Merkle DAGs}
IPFS structures data using Merkle Directed Acyclic Graphs (DAGs). Large files are split into smaller chunks, hashed, and organized into a tree structure. The root hash (CID) uniquely identifies the entire file. This structure allows for parallel downloading of file chunks from different peers, while ensuring data integrity \cite{benet2014ipfs}.

\subsubsection{Blockchain Integration}
IPFS complements blockchain systems by handling the storage layer while the blockchain handles the consensus and state layers. In this model, large data is uploaded to IPFS, and only the resulting fixed-size cryptographic hash (CID) is stored on the blockchain. This guarantees the integrity and availability of the data without increasing the size of the blockchain.
...
To understand the practical application of the consensus mechanisms and data systems discussed previously, in this section we focus on investigating the existing works and show three blockchain architectures: Bitcoin, Ethereum, and Solana. Each represents a different approach to solving the Blockchain Trilemma \cite{buterintrilemma}.
...
\subsection{Ethereum}
Ethereum, which was proposed by Vitalik Buterin in 2013, extended blockchain systems from the peer-to-peer currency model to a distributed Turing-complete system capable of executing arbitrary code while maintaining the functionality of a cryptocurrency \cite{buterin2014ethereum}. Ethereum's architecture can be defined by these
key design choices:
...
\subsection{Solana}
Solana represents a third-generation blockchain designed specifically to maximize throughput without sharding the system. It introduces an architectural design that aims to remove the performance bottlenecks found in Bitcoin and Ethereum \cite{yakovenko2018poh}. Solana's architecture can be defined by these
key design choices:

\begin{itemize}
    \item \textbf{Pipeline Architecture:} Unlike sequential processing in earlier blockchains, Solana employs a pipelined transaction processing architecture in which different stages of transaction are processed concurrently. This architecture improves hardware utilization and contributes to Solana’s high throughput, often cited as tens of thousands of transactions per second under ideal conditions \cite{yakovenko2018poh}. 
...
This chapter established the theoretical foundations necessary for understanding distributed ledger systems. We began by defining the core data structures of blockchain technology, specifically the role of cryptographic hashing in creating tamper-proof histories. We then analyzed the fundamental Public Key Cryptography and Hash Functions that ensure data integrity and authenticity.

Crucially, this chapter explored different attack vectors inherent to decentralized networks, including Sybil attacks, Double-Spending, and 51\% Majority attacks. These risks provided the context for our critical evaluation of consensus mechanisms, where we contrasted the physical cost-based security of Proof of Work (PoW) against the economic incentives of Proof of Stake (PoS), the democratic efficiency of Delegated Proof of Stake (DPoS), and the temporal sequencing of Proof of History (PoH).

Finally, through the analysis of Bitcoin, Ethereum, and Solana, we observed how these theoretical concepts are engineered into real-world production systems, highlighting the trade-offs between security, scalability, and decentralization. This theoretical framework serves as the blueprint for the design and implementation of the educational blockchain system detailed in the upcoming chapters.
