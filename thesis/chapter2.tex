...
A fundamental theoretical constraint in the design of distributed ledger systems is the Blockchain Trilemma. This concept posits that a blockchain system can typically only optimize for two of the three following properties simultaneously: \textbf{Decentralization}, \textbf{Security}, and \textbf{Scalability} \cite{blockchainTrilemma}.

\begin{itemize}
    \item \textbf{Decentralization:} This refers to the distribution of control and decision-making across a wide network of participants (nodes). A highly decentralized system is resistant to censorship and collusion, as no single entity has authority. However, increasing the number of nodes often increases the time required for message propagation and consensus.
    \item \textbf{Security:} This represents the network's ability to resist malicious attacks, such as 51\% attacks, Sybil attacks, or data tampering. In many consensus models, higher security requires redundant validation and substantial economic or computational resources, which can limit throughput.
    \item \textbf{Scalability:} This defines the system's capacity to process a growing volume of transactions per second (TPS) without a degradation in performance (latency). To achieve high scalability, systems often reduce the number of necessary validations or centralize the consensus process, which can negatively impact security or decentralization.
\end{itemize}

This trilemma serves as a method through which different blockchain architectures must be evaluated. For example, traditional Proof of Work systems like Bitcoin prioritize Decentralization and Security, resulting in limited Scalability. Conversely, high-throughput systems may compromise Decentralization by relying on a smaller set of validators. Overcoming this trilemma is the primary motivation behind modern scaling solutions and research papers.\cite{blockchainTrilemma}.
