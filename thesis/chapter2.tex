...
Because hash functions are designed for high sensitivity (Avalanche Effect), the only strategy to find a valid $n$ is via brute-force search. This process requires significant CPU cycles to compute. While calculating the hash of input data is efficient, deriving the original input from a specific hash value is computationally infeasible. Consequently, the resulting “proof” is trivial for other nodes to verify with a single hash operation.
