...
\end{table}
Participants were provided with the \textit{Starter Code} and the laboratory guide as represented in Chapter \ref{ch5_curriculum} and were tasked with completing the five learning modules. They were instructed to state any issues encountered, and their feedback regarding the difficulty of the tasks.
...
\begin{table}[H]
    ...
    M4: Assets & 5/5 (100\%) & Data parsing and validation logic issues were encountered, but all successfully registered and transferred assets. \\ \hline
...
\begin{itemize}
    \item \textbf{The GIL Bottleneck:} Participant P2 faced a major debugging issue where the mining loop blocked network connections due to Python's Global Interpreter Lock (GIL). And he wasn't able to complete the task. This highlighted the need to understand concurrency and Python GIL to keep the network responsive.
    \item \textbf{Algorithmic Complexity:} Participant P4 struggled with the math required for dynamic difficulty adjustment, and wasn't able to complete the task, noting that incorrect settings often caused the blockchain to stop producing blocks.
...
Participants had troubles parsing data, and most participants were challenged with the on-chain validation stage. However, participants reported that this task was easier than Module 3 allowing everyone to successfully register and transfer the digital book assets.

\subsubsection{Module 5: System Extensibility and Design Challenge}
Participant P1 could not complete the schema enforcement, stating an increase in difficulty regarding hash validation compared to previous modules. Participant P5 also found it challenging to correctly calculate transaction sizes for the block size limit.


\section{Discussion} \label{sec:discussion}
This section interprets the results obtained from the two primary contributions of this thesis, the first being the blockchain software solution implemented in Chapter~\ref{ch4_implementation}, and the educational curriculum modules presented in Chapter~\ref{ch5_curriculum}. By synthesizing the quantitative performance metrics from Phase I and the initial small-scale study and participants feedback from Phase II, we assess the extent to which the thesis objectives outlined in Section~\ref{ch1_objectives} have been met.
...
This chapter presented a comprehensive evaluation of the educational blockchain framework, systematically assessing both its technical validity and educational efficacy. Phase I demonstrated that the modular software architecture successfully modeled fundamental distributed systems phenomena, providing empirical evidence for the performance trade-offs between consensus algorithms (PoW vs. PoS) and confirming the system's ability to reproduce theoretical constraints like the Blockchain Trilemma. Phase II validated the curriculum's educational value. High completion rates in foundational modules confirmed the utility of the scaffolding strategy presented in Section~\ref{ch5_repo_structure}, while the specific limitation points identified in advanced concurrency and extensibility tasks highlighted valuable areas for curriculum refinement. Collectively, these findings confirm that the proposed framework serves as an effective platform for advancing blockchain engineering education.