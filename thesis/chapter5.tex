...
\subsection{Problem Statement}
Blockchain systems are inherently constrained by the Blockchain Trilemma (Scalability, Security, Decentralization) \cite{blockchainTrilemma}. To optimize a system, software engineers must first be able to measure the system performance. The core problems addressed in this module are:
\begin{enumerate}
    ...
    \item \textbf{Latency:} How long does it take for a block to be create and propagated from the node, through the network, to a finalized block?
\end{enumerate}
...
The benchmarking suite utilizes a multi-threaded design to stress test the system and to perform post-execution analysis to calculate TPS. Listing~\ref{lst:mod3_stress} below, provide a code snippet of a possible implementation.
