...
The curriculum is designed to transform the learning process for the students from only understanding the theoretical aspects to applying what they have learned to actively engaging with the underlying blockchain system mechanics of consensus, network propagation, and state management. By interacting with the blockchain system developed in the previous chapter, students can transition from passive knowledge consumers to active system engineers.

\section{Approach and Methodology} \label{ch5_pedagogy}

The design of the laboratory exercises is inspired by the theory of Constructionist Learning, proposed by Seymour Papert. This theory states that learning is most effective when students are actively engaged in constructing tangible objects in the real world, rather than acquiring knowledge through passive transmission or consumption~\cite{papert1991situating}. In the context of Computer Science education, this means moving beyond theory, static simulations, or black-box interactions with production networks (such as sending transactions on a public Testnet) toward a deeper system understanding, enabling students and learners to fully understand the concepts by practicing and gaining experience by making code-level modifications to the system.
...
\begin{enumerate}
    ...
    \item \textbf{Tasks:} This is the core phase, where students are tasked to find a solution and modify the codebase in order to resolve the stated problem.
    \item \textbf{Analysis of Findings:} Record an observation of the system's behaviors the student should observe in the system in relation to the actions taken from the previous steps.
...
\begin{enumerate}
    ...
    \item Inspect the current block hashes for the genesis block (block 0), and any subsequent block then examine them with the hashes generated from the original code.
\end{enumerate}

\subsubsection*{Task 2: Network Node Addition} 
To extend the existing network nodes by adding a new node, \texttt{node\_new}, we will use docker and perform the following steps:
\begin{enumerate}
...
The complete reference solution is included in the accompanying repository, specifically within "module\_1" solution directory (refer to Section~\ref{ch5_repo_structure} for repository structure).
...
\begin{itemize}
    \item \textbf{Fork verification:} Students compare the "block\_height" and "last\_hash" in the API of their new node versus a node from the original unmodified codebase. They observe that while both nodes are running, both nodes run on different blockchains. This confirms that the modification to the Genesis configuration isolated the network, and created a fully new chain of blocks.
    \item \textbf{Connectivity Verification:} Students execute \texttt{docker-compose up} and inspect the logs. A successful node creation is indicated when \texttt{node\_new} logs a "Node connection status" log message with the "Current connections" item containing the list of IP addresses and ports associated with nodes 1 to 4. Which confirms that \texttt{node\_new} has successfully established a connection with the other nodes in the network.
\end{itemize}
...
\begin{itemize}
    \item Demonstrate an understanding of the architecture of the (Blockchain) class by designing a mechanism to switch the consensus algorithm of the blockchain system implemented in Chapter~\ref{ch4_implementation} to utilize the Proof of Work (PoW) consensus model described in Section~\ref{ch2_consensus_pow}.
...
The default node behavior is designed for Proof of Stake Time-Bound, which waits for specific slots. The student must alter the \texttt{node.py}, and \texttt{blockchain.py} logic to support the continuous, CPU-Bound nature of Proof of Work, which can be implemented by performing the following tasks:
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item If the node managed to solve the mining problem before the other nodes in the network, then immediately broadcast the mined block to the network upon success.
        \item Ensure that the blockchain network difficulty variable is updated periodically based on blockchain network updates interval (e.g., the difficulty variable is updated after each 5 new blocks generations by the network).
    \end{itemize}
\end{enumerate}
...
\subsubsection*{Task 1: The Consensus Implementation}
The solution involves creating a class that wraps the hashing logic in a while loop. Listing~\ref{lst:mod2_pow_class} below, provide a code snippet of a possible implementation.
...
\newpage
\subsubsection*{Task 2: The Mining Logic Integration}
The Node and Blockchain classes must be updated to handle the mining logic. Listing~\ref{lst:mod2_node_integration}, and Listing~\ref{lst:mod2_blockchain_integration} below, provide a code snippet of a possible implementation.
...
\begin{itemize}
    ...
    \item \textbf{Resource Consumption:} Unlike in module 1, where CPU usage was minimal, the PoW implementation causes an immediate CPU spike. Which the students can observe by reviewing the resources usage statistics on their machine. This, in turn, proves that the security of the network relies on the physical energy process.
...
\subsection{Problem Statement}
Blockchain systems are inherently constrained by the Blockchain Trilemma (Scalability, Security, Decentralization) \cite{buterintrilemma}. To optimize a system, software engineers must first be able to measure the system performance. The core problems addressed in this module are:
\begin{enumerate}
    \item \textbf{Throughput:} How do we measure Transactions Per Second (TPS) in a distributed blockchain system where no single central service exists?
    \item \textbf{Latency:} How long does it take for a transaction to move and propagate from the client, through the node, to a finalized block?
\end{enumerate}

\subsection{Learning Objectives}
By completing this module, students will be able to:
\begin{itemize}
    \item Implement the logic within the Python codebase to capture and measure the different timestamps for the processes of block creation and finalizing a transaction.
    ...
    \item Analyze and create visualization performance metrics to quantify the performance trade-off between the Proof of Stake (PoS) system, which was implemented in Chapter \ref{ch4_implementation} and the Proof of Work (PoW) system, implemented in the second module.    
\end{itemize}
...
To observe network behavior, the student must develop a tool capable of simulating consistent network traffic load and able to calculate the system's throughput under varying load conditions.
...
\subsubsection*{Task 1: Block Propagation Monitoring} 
The solution involves calculating the delay between the block's timestamp and the node's system time upon receipt. Listing~\ref{lst:mod3_metrics} below, provide a code snippet of a possible implementation.
...
The benchmarking suite utilizes a multi-threaded design to decouple transaction generation from network IO. It also performs post-execution analysis to calculate TPS. Listing~\ref{lst:mod3_stress} below, provide a code snippet of a possible implementation.
...
Before executing the full stress test, students confirms that the implemented logging mechanism correctly captures relevant network metrics, by performing the following verifications:
...
\subsubsection*{Task 1: Hello World Transaction}
To support application-specific transactions, the transaction class must include a generic data field, which allows the transaction structure to become flexible enough to support any application logic without changing the underlying consensus mechanism. Listing~\ref{lst:mod4_tx} below, provide a code snippet of a possible implementation of the \texttt{Transaction} class.
...
\subsubsection*{Task 2: Asset Schema}
The \texttt{Book} class acts as a data wrapper. This implementation demonstrates the encapsulation of business logic before it is added to the blockchain.  Listing~\ref{lst:mod4_book} below, provide a code snippet of a possible implementation of the \texttt{Book} class.
...
This final module moves beyond system implementation and introduces students to architectural criticism and extensibility of the system. Having developed an understanding of the blockchain system upon the completion of the previous modules, students are now tasked to critically evaluate the system's limitations and propose scalable design alternatives suitable for real-world deployment. Consequently, this module also functions as an open-ended future work extension of the implemented system.

\subsection{Problem Statement}
In the previous module, students stored book metadata (e.g., title and author) directly on the blockchain. While this approach is acceptable for small, structured data, it exposes a fundamental scalability limitation when applied to large data. This raises the question:\\
\enquote{What are the implications of storing the complete PDF of a book directly on the blockchain?}\\
Blockchains replicate data across all participating nodes. As a result, storing large files of the size of megabytes or gigabytes incurs significant storage increase, bandwidth, and synchronization costs. Over time, this leads to unsustainable ledger growth and threatens the ability of nodes to participate in consensus due to the increased storage overhead to start a node. This practical challenge directly correlates with the data availability problem and the necessity for off-chain storage architectures as mentioned in Section~\ref{ch2_storage}.
...
\begin{enumerate}
    ...
    \item For verification submit a transaction with type \enquote{STORE\_FILE} containing a raw file data. Observe the validation failure in the logs.
\end{enumerate}

\newpage
\subsection{Reference Solution and Analysis}
%  Revise whats under this% 

\subsubsection*{Task 1: Enforcing Block Size Limits} 
This solution demonstrates how to control unsustainable ledger growth, by limiting the number of transactions included in a block during creation. Listing~\ref{lst:mod5_blocksize} below, provide a code snippet of a possible implementation.
...
% \newpage
\subsubsection*{Task 2: Schema Enforcement}
This implementation enforces the application layer to utilize off-chain storage for transactions of type \enquote{STORE\_FILE}. Listing~\ref{lst:mod5_schema} below, provide a code snippet of a possible implementation.
...
\begin{itemize}
    \item \textbf{Rejection of Transactions with Large Data Sizes:} It was observed that although transactions containing large data payloads can be created at the API level, such transactions are rejected by network nodes. This behavior enforces protocol-level limitations and prevents uncontrolled size growth of the blockchain.
...
\subsection{Learning Outcome}
This module demonstrates that blockchain technology should not be regarded as a universal solution for data storage, but rather as a component within a broader system architecture. Through the evaluation of design trade-offs, learners are introduced to key scalability considerations, including the limitations of on-chain data storage. As a result, the module establishes a foundation for advanced scalability concepts such as Layer~2 solutions and off-chain data storage architectures~\cite{poon2017plasma, buterin2021rollups, benet2014ipfs}.
