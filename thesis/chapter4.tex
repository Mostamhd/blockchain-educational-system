...
The full source code for this implementation is open-source and available on GitHub to facilitate student experimentation at the following link: \url{https://github.com/Mostamhd/blockchain-educational-system}
...
\begin{itemize}
    ...
    \item \textbf{API Framework:} \textit{FastAPI} \cite{fastapi} (wrapped via Uvicorn \cite{uvicorn}) is utilized to expose the blockchain's functionality via RESTful endpoints, allowing the communication between the backend node and the frontend client.
    \item \textbf{Cryptography:} The system utilizes the \textit{eth-keys} \cite{eth_keys} and \textit{eth-utils} \cite{eth_utils} libraries. This aligns the project with Ethereum's cryptographic standards, employing \textbf{Keccak-256} for hashing and \textbf{ECDSA} (SECP256k1) for digital signatures, providing students with exposure to production-like Ethereum primitives \cite{wood2014ethereum}.
